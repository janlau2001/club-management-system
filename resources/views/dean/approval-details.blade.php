<x-dashboard-layout>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('dean.approvals') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium">
                ← Back to Approvals
            </a>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        @if($registration->constitution_file)
                            <a href="{{ route('dean.approvals.document', ['registration' => $registration, 'type' => 'constitution']) }}" 
                    ...
                    <div>
                        ...
                        @if($registration->officers_list_file)
                            <a href="{{ route('dean.approvals.document', ['registration' => $registration, 'type' => 'officers']) }}" 
                               class="mt-1 text-sm text-blue-600 hover:text-blue-800" target="_blank">View Document</a>
                        @else
                            <p class="mt-1 text-sm text-gray-500">Not uploaded</p>
                        @endif
                    ...
                    <div>
                        ...
                        @if($registration->activities_plan_file)
                            <a href="{{ route('dean.approvals.document', ['registration' => $registration, 'type' => 'activities']) }}" 
                               class="mt-1 text-sm text-blue-600 hover:text-blue-800" target="_blank">View Document</a>
                        @else
                            <p class="mt-1 text-sm text-gray-500">Not uploaded</p>
                        @endif
                    ...
                    <div>
                        ...
                        @if($registration->budget_proposal_file)
                            <a href="{{ route('dean.approvals.document', ['registration' => $registration, 'type' => 'budget']) }}" 
                               class="mt-1 text-sm text-blue-600 hover:text-blue-800" target="_blank">View Document</a>
                        @else
                            <p class="mt-1 text-sm text-gray-500">Not uploaded</p>
                        @endif
                    ...
                    <div class="flex items-center">
                        ...
                        @else
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-sm text-gray-500">Awaiting VP approval to officially register the club</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3">
                        <button type="button"
                                ...
                                onclick="openEndorseModal('{{ $registration->club_name }}', '{{ route('dean.approvals.approve', $registration) }}')">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            Endorse Registration
                        </button>
                        <button type="button"
            ...
            @elseif($registration->endorsed_by_dean)
                <div class="bg-green-50 border border-green-200 rounded-xl p-6">
                    <div class="flex items-center space-x-3 mb-3">
                        ...
                        <h3 class="text-lg font-semibold text-green-900">Already Endorsed by Dean</h3>
                    </div>
                    <p class="text-green-800 mb-2">You have already endorsed this registration.</p>
                    <p class="text-sm text-green-600">Endorsed on {{ $registration->endorsed_by_dean_at?->format('F j, Y \a\t g:i A') ?? 'N/A' }}</p>
                    <p class="text-sm text-green-600">Endorsed by: {{ $registration->endorsed_by_dean_user ?? 'Dean' }}</p>
                </div>
            @endif
    ...
    <!-- Endorse Registration Modal -->
    <div id="endorseModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mt-4">Endorse Registration</h3>
                <div class="mt-4 mb-6">
                    <p class="text-sm text-gray-500 mb-4" id="endorseMessage">Are you sure you want to endorse this registration?</p>
                    ...
                    <div class="flex items-center text-xs text-red-600">
                        ...
                        <span>Verifying as: <strong>Dean</strong> (Dean)</span>
                    </div>
                </div>

                <div class="text-left">
                    <form id="endorseForm" method="POST">
                        @csrf
                        <div class="mb-6">
                        ...
                        <div class="flex space-x-3">
                            <button type="button" onclick="closeEndorseModal()" 
    ...
    <script>
        function openEndorseModal(clubName, url) {
            document.getElementById('endorseMessage').textContent = 'Are you sure you want to endorse the registration for "' + clubName + '"?';
            document.getElementById('endorseForm').action = url;
