...
<body class="bg-gray-50 font-sans">
    <div class="min-h-screen">
        ...
        @if($club->status === 'suspended')
            <div class="mx-6 mt-4 bg-red-50 border border-red-200 p-6">
        ...
        @if($club->status !== 'suspended' && $club->offense_count > 0)
            <div class="mx-6 mt-4 bg-amber-50 border border-amber-200 p-6">
                <div class="flex items-start">
                    <div class="flex-shrink-0">
                    ...
                    <div class="ml-3 flex-1">
                        ...
                        <div class="mt-4">
                            <a href="{{ route('club.officer.violations') }}"
                               class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium transition-colors">
                                ...
                                @else
                                    ...
                                    @if($clubUser->role === 'officer' || $clubUser->role === 'adviser')
                                        <a href="{{ $club->status === 'suspended' ? '#' : route('club.officer.manage-members') }}"
                                           class="block w-full bg-blue-50 {{ $club->status === 'suspended' ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-100' }} text-blue-700 px-4 py-3 text-sm font-medium transition-colors text-center mb-3 {{ $club->status === 'suspended' ? 'pointer-events-none' : '' }}">
                                            <div class="flex items-center justify-center space-x-2">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        ...
                                        <a href="{{ route('club.officer.view-members') }}"
                                           class="block w-full bg-gray-50 hover:bg-gray-100 text-gray-700 px-4 py-3 text-sm font-medium transition-colors text-center">
                                            <div class="flex items-center justify-center space-x-2">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                                </svg>
                                    ...
                                    @if($clubUser->role === 'officer' || $clubUser->role === 'adviser')
                                        <a href="{{ $club->status === 'suspended' ? '#' : route('club.officer.renewal') }}"
                                           class="block w-full bg-orange-50 {{ $club->status === 'suspended' ? 'opacity-50 cursor-not-allowed' : 'hover:bg-orange-100' }} text-orange-700 px-4 py-3 text-sm font-medium transition-colors text-center {{ $club->status === 'suspended' ? 'pointer-events-none' : '' }}">
                                            <div class="flex items-center justify-center space-x-2">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="border-t border-gray-200 pt-3 mt-3 relative">
                                            ...
                                            <div class="{{ $club->status === 'suspended' ? 'filter blur-sm pointer-events-none' : '' }}">
                                                <a href="{{ route('club.officer.renewal.status') }}"
                                                   class="block w-full bg-purple-50 hover:bg-purple-100 text-purple-700 px-4 py-3 text-sm font-medium transition-colors text-center mb-3">
                                                    <div class="flex items-center justify-center space-x-2">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                        </svg>
                                                ...
                                                <a href="{{ route('club.officer.applicants') }}"
                                                   class="block w-full bg-teal-50 hover:bg-teal-100 text-teal-700 px-4 py-3 text-sm font-medium transition-colors text-center mb-3 relative">
                                                    <div class="flex items-center justify-center space-x-2">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    @if($newApplicationsCount > 0)
                                                        <span class="absolute -top-1 -right-1 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-red-600 min-w-[20px]">
                                                ...
                                                @if($clubUser->role === 'adviser')
                                                    <div class="bg-green-50 border border-green-200 p-3 mt-3">
                                                        <div class="flex items-center">
                                            ...
                                            @if($club->status === 'suspended')
                                                <div class="absolute inset-0 flex items-center justify-center bg-white bg-opacity-90">
                                                    <div class="text-center px-4 py-6">
                                                        <div class="inline-flex items-center justify-center w-12 h-12 bg-red-100 mb-3">
                                                            <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="violationAppealModal" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50">
        <div class="relative top-10 mx-auto border w-[600px] bg-white max-h-[90vh] overflow-y-auto">
            <div class="bg-gray-900 px-6 py-4 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-white">Appeal Club Violations</h3>
                    <p class="text-sm text-gray-300">Submit appeals with supporting documents</p>
                </div>
                <button onclick="closeViolationAppealModal()" class="text-gray-400 hover:text-white">
            ...
            <div class="bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end">
                <button 
                    ...
                    class="px-6 py-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 focus:outline-none transition-colors"
                >
                    Close
                </button>
            </div>
    ...
    <div id="appealFormModal" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50">
        <div class="relative top-10 mx-auto border w-[500px] bg-white">
            <div class="bg-gray-900 px-6 py-4 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-white">Submit Appeal</h3>
                    <p class="text-sm text-gray-300" id="appealViolationTitle">Appeal for violation</p>
                </div>
                <button onclick="closeAppealFormModal()" class="text-gray-400 hover:text-white">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
            ...
            <form id="appealForm" enctype="multipart/form-data">
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea 
                            ...
                            required 
                            rows="4" 
                            class="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:border-gray-900 transition-colors resize-none"
                ...
                <div class="bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end space-x-3">
                    ...
                    <button 
                        type="submit" 
                        class="px-6 py-2 bg-gray-900 text-white hover:bg-gray-800 focus:outline-none transition-colors font-medium"
                    >
                        Submit Appeal
                    </button>
                </div>
                ...
                <div class="border border-gray-200 p-4 space-y-3">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="ml-4 flex flex-col items-end space-y-2">
                            <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium ${violation.severity_color}">
                                ${violation.severity.charAt(0).toUpperCase() + violation.severity.slice(1)}
                            </span>
                            <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium ${violation.status_color}">
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                        ...
                        <div class="space-x-2">
                            ...
                            ` : violation.appeal_status ? `
                                <span class="px-3 py-1 text-xs ${violation.appeal_status_color}">
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            ...
                                            <div class="mt-2 flex items-center space-x-4 text-xs text-gray-500">
                                                ...
                                                <span class="px-2 py-1 ${violation.severity_color}">${violation.severity}</span>
                                                <span class="px-2 py-1 ${violation.status_color}">${violation.status}</span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                            ${violation.can_appeal ? 
                                                ...
                                                violation.appeal_status ? 
                                                `<span class="px-2 py-1 text-xs ${violation.appeal_status_color}">Appeal ${violation.appeal_status}</span>` :
    ...
    <!-- Notification Modal -->
    <div id="notificationModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm overflow-y-auto h-full w-full hidden" style="z-index: 9999;">
        <div class="relative top-20 mx-auto p-5 border w-96 bg-white">
            <div class="mt-3">
                <div id="notificationIcon" class="mx-auto flex items-center justify-center h-12 w-12">
                ...
                <div class="mt-2 px-7 py-3">
                    ...
                    <div class="flex items-center justify-center mt-4">
                        <button type="button" onclick="hideNotificationModal()" 
                                class="px-4 py-2 bg-gray-900 text-white hover:bg-gray-800 transition-colors">
                            OK
                        </button>
                    </div>
                </div>
    ...
    <script>
        function showNotificationModal(title, message, type = 'success') {
            ...
            if (type === 'success') {
                icon.className = 'mx-auto flex items-center justify-center h-12 w-12 bg-green-100';
                ...
                icon.className = 'mx-auto flex items-center justify-center h-12 w-12 bg-red-100';
                ...
                icon.className = 'mx-auto flex items-center justify-center h-12 w-12 bg-yellow-100';
