...
<body class="bg-gray-50">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="{{ route('club.officer.member.edit', $clubUser) }}" 
                           class="bg-white text-green-800 hover:bg-gray-50 px-4 py-2 text-sm font-medium transition-colors">
                            Edit Member
                        </a>
                    </div>
                </div>
        ...
        <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div class="ml-6">
                            ...
                            <div class="flex items-center mt-2 space-x-3">
                                ...
                                @elseif($clubUser->role === 'officer')
                                    <span class="inline-flex items-center px-3 py-1 text-sm font-medium bg-purple-100 text-purple-800">
                                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                @else
                                    <span class="inline-flex items-center px-3 py-1 text-sm font-medium bg-blue-100 text-blue-800">
                                        <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                @if($clubUser->id === $currentUser->id)
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800">
                                        This is you
                                    </span>
                                @endif
                            </div>
            ...
            <div class="bg-white border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('club.officer.member.edit', $clubUser) }}" 
                           class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 font-medium transition-colors flex items-center">
                        ...
                        @if($clubUser->id !== $currentUser->id)
                            <button onclick="showRemoveModal({{ $clubUser->id }}, '{{ $clubUser->name }}')" 
                                    class="bg-red-600 hover:bg-red-700 text-white px-6 py-3 font-medium transition-colors flex items-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <span class="bg-gray-300 text-gray-500 px-6 py-3 font-medium cursor-not-allowed flex items-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Remove Member Modal -->
    <div id="removeModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm overflow-y-auto h-full w-full hidden" style="z-index: 1000;">
        <div class="relative top-20 mx-auto w-96 bg-white border border-gray-200">
            <div class="bg-gray-900 px-6 py-4">
                <h3 class="text-lg font-medium text-white">Remove Member</h3>
            </div>
            <div class="px-6 py-4">
                ...
                <form method="POST" action="{{ route('club.officer.member.remove', $clubUser) }}" class="mt-4">
                    ...
                    <div class="bg-gray-50 border-t border-gray-200 px-6 py-4 -mx-6 -mb-4">
                        <div class="flex items-center justify-end space-x-3">
                            <button type="button" onclick="hideRemoveModal()" 
                                    class="px-4 py-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">
                                Cancel
                            </button>
                            <button type="submit" 
