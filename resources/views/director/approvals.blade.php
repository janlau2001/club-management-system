<x-dashboard-layout>
    <x-slot name="title">Director Approvals</x-slot>

    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Director Approvals</h1>
        <p class="text-gray-600 mt-2">Review and note club registration applications</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                <!-- Pending Tab -->
                <a href="{{ route('director.approvals', ['status' => 'pending']) }}" 
                ...
                <!-- Approved Tab -->
                <a href="{{ route('director.approvals', ['status' => 'approved']) }}" 
                   class="group inline-flex items-center py-4 px-6 border-b-2 font-medium text-sm transition-colors
        ...
        <div class="px-6 py-3 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <p class="text-sm text-gray-600">
                    @if($status === 'pending')
                        <span class="font-medium text-gray-900">{{ $registrations->count() }}</span> pending registration(s) - You can only note applications after PSG Council approval
                    @else
                        <span class="font-medium text-gray-900">{{ $registrations->count() }}</span> registration(s) noted by you
                    @endif
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-6 border-b border-gray-100">
            <h2 class="text-xl font-semibold text-gray-900">Director Approvals</h2>
            <p class="text-gray-600 mt-1">Applications ready for Director noting (PSG Council-approved applications)</p>
        </div>
                        ...
                        @foreach($registrations as $registration)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('director.approvals.show', $registration) }}" 
                                       class="text-blue-600 hover:text-blue-900 mr-3">
                                        View Details
                                    </a>
                                    @if(!$registration->noted_by_director && $registration->status !== 'rejected')
