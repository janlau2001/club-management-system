...
<a href="{{ route('club.officer.dashboard') }}" class="flex items-center px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
...
<a href="{{ route('club.officer.manage-members') }}" class="flex items-center px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<a href="{{ route('club.officer.club-renewal') }}" class="flex items-center px-4 py-3 text-white bg-white/10 rounded-lg">
    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <span class="ml-auto px-2 py-1 text-xs bg-red-500 text-white rounded-full">Action Required</span>
    @endif
</a>

<a href="{{ route('club.officer.profile') }}" class="flex items-center px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="flex items-center space-x-3">
    @if($renewal && (($clubUser->role === 'president' && $renewal->status === 'pending') || ($clubUser->role === 'adviser' && $renewal->status === 'prepared')))
    <button @click="showRenewalModal = true" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center">
    ...
    @endif
    <a href="{{ route('club.officer.dashboard') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center">
        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="space-y-6" x-data="renewalDashboard()">
    ...
    @if($renewal && (($clubUser->role === 'president' && $renewal->status === 'pending') || ($clubUser->role === 'adviser' && $renewal->status === 'prepared')))
    <div x-show="showRenewalModal" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <form method="POST" action="{{ route('club.officer.renewal.submit') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                        ...
                        <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                            ...
                            @if(in_array($clubUser->position, ['President', 'Adviser']) || $clubUser->role === 'adviser')
                                <button type="submit" 
                                        class="px-6 py-2 bg-gradient-to-r from-orange-600 to-orange-700 text-white rounded-lg hover:from-orange-700 hover:to-orange-800 transition-all duration-200 font-medium">
                                    Submit Renewal Application
                                </button>
                            @else
                                <button type="button" disabled
                                        class="px-6 py-2 bg-gray-300 text-gray-500 rounded-lg cursor-not-allowed font-medium">
                                    Access Restricted
                                </button>
                            @endif
                        </div>
